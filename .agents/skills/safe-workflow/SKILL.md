...
> **TEMPLATE**: This skill uses `TASK` as a placeholder. Replace with your project's ticket prefix (e.g., `WOR`, `PROJ`, `FEAT`).
...
```text
# Good
TASK-447-create-safe-workflow-skill
...
```text
feat(harness): create safe-workflow skill [TASK-447]
fix(auth): resolve login redirect [TASK-57]
```
...
# 3. Make commits
git commit -m "type(scope): description [TASK-XXX]"
...
4. CI passes: `run your test suite`
